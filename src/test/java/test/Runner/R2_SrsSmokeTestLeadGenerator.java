...
@CucumberOptions(
		format = { "pretty", "html:target/cucumber","json:target/JSON/LeadGenOutputRerun.json" },
        ...
        plugin = {"com.cucumber.listener.ExtentCucumberFormatter:target/SRSDashboardReportReRun.html","rerun:target/SrsLeadGenRerun2.txt"}
...
public class R2_SrsSmokeTestLeadGenerator extends WEBHelper{
	...
	{		
		...
		if (br.readLine() == null){
			...
			Mail.SendReport("SRSDashboardReportReRun.html", GetApplication() + GetTestEnv(), "[SMOKE TEST - RERUN]: " + GetApplication() + GetTestEnv() + " (LeadGen:"+testStatus+") - ");
